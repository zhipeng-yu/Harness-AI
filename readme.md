...
第四章含 39 节讲解、约 1.48 万字正文与三幅灰度方法示意，练习是将一个真实瓶颈转成可传递流程，试跑后检验产出放大与判断上移。见 [第四章内容说明](docs/chapter-04-review.md)；本机入口为 <http://127.0.0.1:3001/chapters/chapter-04>。

课程采用博客式分页阅读：先了解路线，再逐节阅读解释与例子，最后回顾概念、场景和误区并进入实践。每节独立一页，正文完整显示，补充例子明确标注为说明性例子。页面保留浅灰画布与白色圆角面板，正文为主、灰度方法示意为辅，高度随内容展开，窄窗口自动变为单栏。支持目录跳转、按钮和方向键翻页，最后一页进入反思、行动与 Artifact 工作区；当前阅读页不持久化。

Artifact 在章节页中称为“实践成果卡”：创建后立即显示四项正文，草稿可以继续编辑；开始实践时锁定当前版本，复盘与生成的下一版会留在同一页的版本历史中。

“教学诊断”用于承接已经发现的教学异常。可在 `/diagnosis` 手动创建作业、出勤或退费/投诉案例，按“看变化—查证据—作判断—去验证—定动作—看结果”六步填写；内容自动保存，完成后生成“问题—证据—判断—动作—结果”只读摘要。诊断不接入外部系统，也不自动判断老师问题。
...
登录、多用户共享、远程访问、部署、运行时 AI、CMS、文件上传、自动分析聊天记录、自动判断老师问题、提醒、积分、考试、排行和社区。未来若分享给不超过 20 人，再单独设计数据隔离；现在不提前实现。
