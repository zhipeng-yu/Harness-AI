...
当前已有 18 章目录，第一章《获取超体的脑机接口：获得 10 倍输出能力》已发布，其余 17 章尚未发布，正式目录状态仍为 `awaiting_audio`。
第二章音频已完成本地转录，约 40:32 为课程结尾，40:35 后为多录静音。正文与四张插画已准备好，双击 `artifacts/review/chapter-02.html` 审阅；详见[第二章待审稿说明](docs/chapter-02-review.md)，审批后再发布。其余 16 章仍等待音频。
第二章待审稿目前仅本地提交；远端为公开仓库，自动审批要求先取得该课程内容公开推送的明确授权。

已发布章节的知识讲解会按现有语义结构自动整理成桌面 16:9 幻灯片；第一章共 18 页，包含与页面知识点对应的日系清新卡通插画和方向键翻页。最后一页进入原有反思、行动与 Artifact 工作区。当前不做手机专项适配。
